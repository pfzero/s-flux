 ...
 * });
 */
const DEFAULT_CHANGE_HANDLER = 'onChange';
...
const ListenerMixin = {
     ...
     * @method getListener
     */
    getListener(store, handler) {
        handler = this.getHandler(handler);
        const storeInstance = this.getStore(store);
        return {
            store: storeInstance,
            handler: handler
        };
